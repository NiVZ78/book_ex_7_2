...
static void canvas_update_proc(Layer *this_layer, GContext *ctx) {
  ...
  int digit_height = (5 * tile_size) + 4;
    ...
    // top left
    draw_digit(ctx, digit_array[1], s_main_window_center.x-digit_width-(tile_size/2), s_main_window_center.y-digit_height-(tile_size/2));  
    
    // top right
    draw_digit(ctx, digit_array[2], s_main_window_center.x+(tile_size/2), s_main_window_center.y-digit_height-(tile_size/2));  
    
    // bottom left
    draw_digit(ctx, digit_array[3], s_main_window_center.x-digit_width-(tile_size/2), s_main_window_center.y+(tile_size/2));  
  
    // bottom right
    draw_digit(ctx, digit_array[4], s_main_window_center.x+(tile_size/2), s_main_window_center.y+(tile_size/2));  
